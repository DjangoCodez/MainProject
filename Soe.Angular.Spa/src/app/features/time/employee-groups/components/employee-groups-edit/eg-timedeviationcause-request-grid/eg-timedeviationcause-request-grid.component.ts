...
@Component({
  selector: 'soe-eg-timedeviationcause-request-grid',
  standalone: false,
  templateUrl:
    '../../../../../../shared/ui-components/grid/grid-wrapper/embedded-grid-wrapper-template.html',
  providers: [FlowHandlerService, ToolbarService],
})
export class EgTimedeviationcauseRequestGridComponent
  ...
  implements OnInit
{
  ...
  override ngOnInit(): void {
    ...
    this.startFlow(
      ...
      'time.employee.employeegroups.timedeviationcauserequests.grid'
    );
    this.form.timeDeviationCauseRequestIds.valueChanges.subscribe(v =>
      this.initRows(v)
    );
  }
    ...
    this.toolbarService.createItemGroup({
      ...
      items: [
        this.toolbarService.createToolbarLabel('label', {
          labelKey: signal(
            'time.employee.employeegroup.absencecauses.request.linked'
          ),
        }),
      ],
    });
  ...
  override addRow(): void {
    ...
    super.addRow(row, this.form.timeDeviationCauseRequestIds, IdForm);
  }

  override deleteRow(row: any) {
    super.deleteRow(row, this.form.timeDeviationCauseRequestIds);
  }
}
