import { MatrixDefinitionColumn } from '@shared/models/matrix.model';
...
export class ExportDefinitionGridDTO implements IExportDefinitionGridDTO {
  exportDefinitionId: number;
  actorCompanyId: number;
  name: string;
  type?: number;
  ...
  implements IExportDefinitionLevelColumnDTO
{
  ...
  position: number;
  defaultValue: string;
  matrixDefinitionColumn: MatrixDefinitionColumn;
  isDelimiter: boolean;
  isPosition: boolean;
  fillChar: string;
  ...
  constructor() {
    ...
    this.position = 0;
    this.defaultValue = '';
    this.matrixDefinitionColumn = new MatrixDefinitionColumn();
