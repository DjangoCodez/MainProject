...
export default function getDocDefinition(printParams: any, agGridApi: GridApi) {
  const {
    PDF_HEADER_COLOR,
  ...
  function getColumnGroupsToExport() {
    const displayedColumnGroups = agGridApi.getAllDisplayedColumnGroups();
  ...
  function getColumnsToExport() {
    ...
    agGridApi.getAllDisplayedColumns().forEach((col: any) => {
  ...
  function getRowsToExport(columnsToExport: any[]) {
    ...
    agGridApi.forEachNodeAfterFilterAndSort((node: any) => {
      ...
      const rowToExport = columnsToExport.map(({ colId }) => {
        const cellValue = agGridApi.getCellValue({
          rowNode: node,
          colKey: colId,
        });
  ...
  function getPdfExportOptions(colId: any) {
    const col = agGridApi.getColumn(colId);
    return (<any>col).colDef.pdfExportOptions;
  }
}
