...
export class TranslateUtil {
  static getDefaultLocaleText(translate: TranslateService): {
    [key: string]: string;
  } {
    let localeText = {};
    switch (translate.getCurrentLang()) {
      case 'da-DK':
  ...
  static getCustomLocaleText(
    translate: TranslateService
  ): Observable<{ [key: string]: string }> {
    // Get all keys from both English and SOE locale objects
    // SOE keys are custom keys that need translation
    const agGridKeys = [
      ...Object.keys(TranslateUtil.AG_GRID_LOCALE_EN),
      ...Object.keys(TranslateUtil.AG_GRID_LOCALE_SOE),
    ];
    const translationObservables = agGridKeys.map(key =>
      translate.get('aggrid.' + key)
    );

    // Use forkJoin to combine all translation observables into one
    return forkJoin(translationObservables).pipe(
      map(values => {
        const result: { [key: string]: string } = {};
        agGridKeys.forEach((key, idx) => {
          // Only add to result if translation exists and is not the same as the key
          if (values[idx] && values[idx] !== 'aggrid.' + key)
  ...
  static AG_GRID_LOCALE_SOE = {
    'soe.clearFilters': '',
    'soe.export': '',
    'soe.exportAllExcel': '',
    'soe.exportFilteredExcel': '',
    'soe.exportAllCsv': '',
    'soe.exportFilteredCsv': '',
    'soe.exportAllPdf': '',
    'soe.exportAllExcelTable': '',
    'soe.exportFilteredExcelTable': '',
    'soe.gridState': '',
    'soe.gridStateSaveDefault': '',
    'soe.gridStateDeleteDefault': '',
    'soe.gridStateSave': '',
    'soe.gridStateDefault': '',
    'soe.gridStateDelete': '',
    'soe.resizeToFit': '',
    'soe.resizeAutoAllAndHeaders': '',
    'soe.resizeAutoAllExceptHeaders': '',
  };

  // Example locale file for English, as of version 32.1.0
  // Can be found in \node_modules\@ag-grid-community\locale\dist\umd\@ag-grid-community\locale.js
  static AG_GRID_LOCALE_EN = {
    // Set Filter
