...
import { Feature, TermGroup, TermGroup_ReportUserSelectionAccessType } from "../../../Util/CommonEnumerations";
import { CoreUtility } from "../../../Util/CoreUtility";
import { SOEMessageBoxButtons, SOEMessageBoxImage } from "../../../Util/Enumerations";
import { UserSelectionAccessDTO, UserSelectionDTO } from "../../Models/UserSelectionDTOs";

export class SaveUserSelectionDialogController {
    ...
    private selection: UserSelectionDTO;
    ...
    constructor(
        ...
        selection: UserSelectionDTO) {

        this.isNew = !selection || !selection.userSelectionId;
        this.initiallyPrivate = !this.isNew && selection && !!selection.userId;

        this.selection = new UserSelectionDTO();
    ...
    private save() {
        ...
        if (this.accessTypeIsRole) {
            this.roles.filter(r => r['selected']).map(r => r.id).forEach(id => {
                let acc = new UserSelectionAccessDTO();
        ...
        this.$uibModalInstance.close({ selection: this.selection, accessType: this.accessType });
    }
