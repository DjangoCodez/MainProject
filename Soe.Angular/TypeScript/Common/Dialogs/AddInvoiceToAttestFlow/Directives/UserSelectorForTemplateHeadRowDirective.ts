...
import { IAttestWorkFlowTemplateRowDTO, IAttestWorkFlowHeadDTO, IUserSmallDTO, ISmallGenericType, IAttestWorkFlowRowDTO, IAttestRoleDTO } from "../../../../Scripts/TypeLite.Net4";
...
export class UserSelectorForTemplateHeadRowDirectiveFactory {
    ...
    public static create(translationService: ITranslationService, urlHelperService: IUrlHelperService): ng.IDirective {
        return {
            templateUrl: urlHelperService.getGlobalUrl('Common/Dialogs/AddInvoiceToAttestFlow/Directives/UserSelectorForTemplateHeadRow.html'),
            scope: {
                row: '=',
                head: '=',
                mode: '=',
...
export class UserSelectorForTemplateHeadRowDirectiveController extends GridControllerBase {
    ...
    public $onInit() {
        ...
        this.soeGridOptions.enableFullRowSelection = true;
    ...
    protected setupGrid() {
        this.$q.all([this.loadTerms(), this.loadAttestWorkFlowTypes(), this.loadAttestWorkFlowUsers(), this.loadAttestWorkFlowAttestRoles()]).then(() => this.setupGridColumns());
    }
    ...
    private loadAttestWorkFlowUsers(): ng.IPromise<any> {
        return this.supplierService.getAttestWorkFlowUsersByAttestTransitionId(this.row.attestTransitionId).then((data: IUserSmallDTO[]) => {
            this.checkableUsers = data.map(u => new Checkable(u));

            if (!this.head.rows)
                return;

            var relevantRows = this.head.rows.filter(r => r.attestTransitionId === this.row.attestTransitionId && (<any>r.processType) !== TermGroup_AttestWorkFlowRowProcessType.Registered);
    ...
    private loadAttestWorkFlowAttestRoles(): ng.IPromise<any> {
        return this.supplierService.getAttestWorkFlowAttestRolesByAttestTransitionId(this.row.attestTransitionId).then((data: IAttestRoleDTO[]) => {
