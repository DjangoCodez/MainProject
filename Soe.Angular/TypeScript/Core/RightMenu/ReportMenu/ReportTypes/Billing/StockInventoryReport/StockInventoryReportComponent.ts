...
import { BoolSelectionDTO, DateRangeSelectionDTO, DateSelectionDTO, IdSelectionDTO, TextSelectionDTO } from "../../../../../../Common/Models/ReportDataSelectionDTO";
...
export class StockInventoryReport {
    ...
    public static component(): ng.IComponentOptions {
        return {
            controller: StockInventoryReport,
            templateUrl: soeConfig.baseUrl + "Core/RightMenu/ReportMenu/ReportTypes/Billing/StockInventoryReport/StockInventoryReportView.html",
            bindings: {
                userSelection: "=",
                selections: "<",
            }
    ...
    public static componentKey = "stockInventoryReport";
    ...
    private selectedStockValueDate: DateSelectionDTO;
    ...
    constructor(private $scope: ng.IScope, $timeout: ng.ITimeoutService, private translationService: ITranslationService, private reportDataService: IReportDataService, private coreService: ICoreService) {
        ...
        const keys = [
            ...
            "common.report.selection.stockvaluedate"
        ]
    ...
    private setSavedUserFilters(savedValues: ReportUserSelectionDTO) {
        this.selectedStockInventoryItem = this.userSelection.getIdSelection(Constants.REPORTMENU_SELECTION_KEY_STOCK_INVENTORY);
        ...
        this.selectedStockValueDate = this.userSelection.getDateSelectionFromKey(Constants.REPORTMENU_SELECTION_KEY_STOCK_VALUE_DATE);
    }

    private getStockInventories() {
        this.selectableStockInventoryList = [];
        return this.reportDataService.getStockInventories().then((x) => {
            x = x.sort((a, b) => {
                const start1 = new Date(a.inventoryStart)
                const start2 = new Date(b.inventoryStart)
                return start1 > start2 ? -1 : 1;
            });

            this.selectableStockInventoryList.push({ id: 0, name: ' ' });
            x.forEach(y => {
                this.selectableStockInventoryList.push({ id: y.stockInventoryHeadId, name: y.headerText + " (" + new Date(y.inventoryStart).toFormattedDate() + ")" });
            });
        });
    }
    ...
    public onDateSelected(dateSelection: DateSelectionDTO) {
        this.selections.upsert(Constants.REPORTMENU_SELECTION_KEY_STOCK_VALUE_DATE, dateSelection);
    }
