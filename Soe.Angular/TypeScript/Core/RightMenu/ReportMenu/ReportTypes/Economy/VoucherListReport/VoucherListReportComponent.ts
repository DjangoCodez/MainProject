...
export class VoucherListReport {
    public static component(): ng.IComponentOptions {
        return {
            controller: VoucherListReport,
            templateUrl: soeConfig.baseUrl + "Core/RightMenu/ReportMenu/ReportTypes/Economy/VoucherListReport/VoucherListReportView.html",
            bindings: {
                userSelection: "=",
                selections: "<"
            }
    ...
    public static componentKey = "voucherListReport";
    ...
    private voucherSeriesList: ISmallGenericType[] = [];
    ...
    private voucherNumberChangedFrom(selection: TextSelectionDTO) {
        this.selections.upsert(Constants.REPORTMENU_SELECTION_KEY_VOUCHER_SERIES_NUMBER_FROM, selection);
    }

    private voucherNumberChangedTo(selection: TextSelectionDTO) {
        this.selections.upsert(Constants.REPORTMENU_SELECTION_KEY_VOUCHER_SERIES_NUMBER_TO, selection);
    }

    private voucherSeriesChangedFrom(selection: IdSelectionDTO) {
        this.selections.upsert(Constants.REPORTMENU_SELECTION_KEY_VOUCHER_SERIES_FROM, selection);
    }

    private voucherSeriesChangedTo(selection: IdSelectionDTO) {
        this.selections.upsert(Constants.REPORTMENU_SELECTION_KEY_VOUCHER_SERIES_TO, selection);
    }
    ...
    public getVoucherSeries() {
        this.voucherSeriesList = [];
        const mapIterator = this.selections.materialize().entries();
        let current = mapIterator.next();
        while (!current.done) {
            const [key, entry] = current.value;
            if (entry.key == 'accountPeriodFrom') {
                this.fromYearId = entry.yearId;
            }
            else if (entry.key == 'accountPeriodTo') {
                this.toYearId = entry.yearId;
            }
            current = mapIterator.next();
        }
        if (this.fromYearId > 0 && this.toYearId > 0) {
            this.reportDataService.getVoucherSeries(this.fromYearId, this.toYearId, false).then(data => {
                this.voucherSeriesList = [];
                _.forEach(data, (name, id) => {
                    this.voucherSeriesList.push(new SmallGenericType(parseInt(id), name))
                });
                if (this.userSelection) {
                    if (this.userSelection.getIdSelection(Constants.REPORTMENU_SELECTION_KEY_VOUCHER_SERIES_FROM)) {
                        this.voucherSeriesFrom = _.find(this.voucherSeriesList, d => d.id === this.userSelection.getIdSelection(Constants.REPORTMENU_SELECTION_KEY_VOUCHER_SERIES_FROM).id);
                        this.voucherSeriesChangedFrom(new IdSelectionDTO(this.voucherSeriesFrom.id));
                    }
                    if (this.userSelection.getIdSelection(Constants.REPORTMENU_SELECTION_KEY_VOUCHER_SERIES_TO)) {
                        this.voucherSeriesTo = _.find(this.voucherSeriesList, d => d.id === this.userSelection.getIdSelection(Constants.REPORTMENU_SELECTION_KEY_VOUCHER_SERIES_TO).id);
                        this.voucherSeriesChangedTo(new IdSelectionDTO(this.voucherSeriesTo.id));
                    }
                    this.voucherNumberFrom = this.userSelection.getTextSelection(Constants.REPORTMENU_SELECTION_KEY_VOUCHER_SERIES_NUMBER_FROM);
                    this.voucherNumberTo = this.userSelection.getTextSelection(Constants.REPORTMENU_SELECTION_KEY_VOUCHER_SERIES_NUMBER_TO);
                }
