...
import { Feature, CompanySettingType, TermGroup_Languages, SoeInvoiceMatchingType, SoeOriginType, OrderInvoiceRegistrationType, TermGroup_BillingType, CustomerAccountType, AccountingRowType, SoeEntityState, SoeInvoiceType } from "../../../../Util/CommonEnumerations";
...
export class EditController extends GridControllerBase implements IEditControllerBase {
    ...
    constructor($http,
        ...
        super("Soe.Economy.Customer.Invoice.Matches", "economy.supplier.invoice.matches.supplier", Feature.Economy_Customer_Invoice_Matching, $http, $templateCache, $timeout, $uibModal, coreService, translationService, urlHelperService, messagingService, notificationService, uiGridConstants);
    ...
    public setupMatchesGrid() {
        ...
        this.translationService.translateMany(keys).then((terms) => {
            ...
            super.addColumnNumber("invoiceTotalAmount", terms["economy.supplier.invoice.matches.totalamount"], "10%", null, null, "");
            super.addColumnNumber("invoicePayedAmount", terms["economy.supplier.invoice.matches.totalmatchamount"], "10%", null, null, "");
            ...
            if (this.customerInvoicePermission)
                this.soeGridOptions.addColumnEdit(terms["core.edit"], "openCustomerInvoice");
    ...
    private loadMatchCodes(): ng.IPromise<any> {
        return this.commonCustomerService.getMatchCodes(SoeInvoiceMatchingType.CustomerInvoiceMatching, false).then(
            x => {
                this.matchCodes = x;
            });
    }
    ...
    private loadVoucherSeries(): ng.IPromise<any> {
        return this.commonCustomerService.getVoucherSeriesByYear(this.accountYearId, false)
            .then((x) => {
    ...
    private loadDefaultVoucherSeriesId() {
        return this.commonCustomerService.getDefaultVoucherSeriesId(this.accountYearId, CompanySettingType.CustomerInvoiceVoucherSeriesType).then((x) => {
        ...
        if (this.validate()) {
            ...
            modal.result.then(val => {
                if (val) {
                    ...
                    this.startSave();
                    this.commonCustomerService.InvoicePaymentMatchAndVoucher(invoicePaymentMatchAndVoucherDTO).then((result) => {
                        if (result.success) {
                            this.accountingRows = [];
                            ...
                            this.completedSave(this.customerId);
                        } else {
    ...
    public match(model: any): void {
        ...
        // Credit row
        var remainingAmount = this.soeGridOptions.getSelectedRows().filter(f => f.type === SoeOriginType.CustomerInvoice).map(f => f.invoiceMatchAmount).reduce((current, previous) => {
            return current + previous;
        }, 0);

        this.createAccountingRow(CustomerAccountType.Unknown, this.defaultDebitAccountId, remainingAmount, remainingAmount > 0, false, false);
        ...
        if (remainingAmount !== 0) {
            if (remainingAmount > 0) {
                this.createAccountingRow(CustomerAccountType.Unknown, model.accountId || this.defaultCreditAccountId, remainingAmount, true, false, false);
            } else {
                if (this.accountVat) {
                    var vatRate = this.vatRate / (100 + this.vatRate);
                    var vatAmount = remainingAmount * vatRate;
                    var credAmount = remainingAmount * (1 - vatRate);
                    this.createAccountingRow(CustomerAccountType.Unknown, model.accountId || this.defaultCreditAccountId, credAmount, true, false, false);
                    this.createAccountingRow(CustomerAccountType.Unknown, model.vatAccountId || this.defaultVatAccountId, vatAmount, true, true, false);
                } else {
                    this.createAccountingRow(CustomerAccountType.Unknown, model.accountId || this.defaultCreditAccountId, remainingAmount, true, false, false);
                }
            }
    ...
    private loadAccountPeriod(accountYearId: number): ng.IPromise<any> {
        return this.commonCustomerService.getAccountPeriodId(accountYearId, this.selectedDate).then((id: number) => {
    ...
    private createAccountingRow(type: CustomerAccountType, accountId: number, amount: number, isDebitRow: boolean, isVatRow: boolean, isContractorVatRow: boolean): AccountingRowDTO {
        ...
        row.isInterimRow = type === CustomerAccountType.Debit;
    ...
    public search() {
        this.commonCustomerService.getInvoicePaymentsMatches(this.customerId, SoeInvoiceType.CustomerInvoice).then((result: any[]) => {
    ...
    updateSelectedInfo(): void {
        var invoiceAmount = this.soeGridOptions.getSelectedRows().filter(f => f.type === SoeOriginType.CustomerInvoice).map(f => f.invoiceMatchAmount).reduce((current, previous) => {
            return current + previous;
        }, 0);
        var paymentAmount = this.soeGridOptions.getSelectedRows().filter(f => f.type === SoeOriginType.CustomerPayment).map(f => f.invoiceMatchAmount).reduce((current, previous) => {
            return current + previous;
        }, 0);
    ...
    setTotatlInfo(): void {
        var invoiceAmount = this.soeGridOptions.getData().filter(f => f.type === SoeOriginType.CustomerInvoice).map(f => f.invoiceTotalAmount).reduce((current, previous) => {
            return current + previous;
        }, 0);
        var paymentAmount = this.soeGridOptions.getData().filter(f => f.type === SoeOriginType.CustomerPayment).map(f => f.invoiceTotalAmount).reduce((current, previous) => {
            return current + previous;
        }, 0);
        var totalAmount = invoiceAmount + paymentAmount;
