...
export class InvoiceMatchMatchingController extends GridControllerBase {
    ...
    constructor($http,
        ...
        super("economy.customer.invoice.attestaccountingrows",
            "economy.customer.invoice.attestaccountingrows",
    ...
    public updateData() {
        if (this.matching) {
            var id = (this.matching.type === SoeEntityType.CustomerInvoice) ? this.matching.invoiceId : this.matching.paymentRowId;
            this.commonCustomerService.getInvoicesMatches(id, this.matching.actorId, this.matching.type)
                .then(x => {
                });
        }
    }
