...
import { TimeCodeAbsenceDTO } from "../../../Common/Models/TimeCode";
import { Feature, SoeTimeCodeType, TermGroup, TermGroup_TimeCodeRuleType, SoeEntityState } from "../../../Util/CommonEnumerations";
...
export class EditController extends EditControllerBase2 implements ICompositionEditController {
    ...
    public onInit(parameters: any) {
        ...
        this.flowHandler.start([{ feature: Feature.Time_Preferences_TimeSettings_TimeCodeAbsense_Edit, loadReadPermissions: true, loadModifyPermissions: true }]);
    }

    private onAllPermissionsLoaded(response: IPermissionRetrievalResponse) {
        this.readOnlyPermission = response[Feature.Time_Preferences_TimeSettings_TimeCodeAbsense_Edit].readPermission;
        this.modifyPermission = response[Feature.Time_Preferences_TimeSettings_TimeCodeAbsense_Edit].modifyPermission;
    }
    ...
    private loadTerms(): ng.IPromise<any> {
        var keys: string[] = [
            "core.time.minutes",
            "time.time.timecodeabsences.timecodeabsence"
        ];
    ...
    private loadRegistrationTypes(): ng.IPromise<any> {
        return this.coreService.getTermGroupContent(TermGroup.TimeCodeRegistrationType, true, false).then(x => {
    ...
    private loadAdjustQuantityTimeCodes(): ng.IPromise<any> {
        this.timeCodeAbsences = [];
        this.timeCodeAbsences.push(new SmallGenericType(0, ''));
        return this.timeService.getTimeCodes(SoeTimeCodeType.Absense, false, false).then((x) => {
            _.forEach(x, (t) => {
                this.timeCodeAbsences.push(new SmallGenericType(t.timeCodeId, t.name));
            });
        });
    }
    ...
    private load(): ng.IPromise<any> {
        return this.timeService.getTimeCode(SoeTimeCodeType.Absense, this.timeCodeId, true, true).then(x => {
            this.isNew = false;
            this.timeCode = x;
            this.dirtyHandler.clean();
            this.messagingHandler.publishSetTabLabel(this.guid, this.terms["time.time.timecodeabsences.timecodeabsence"] + ' ' + this.timeCode.name);
        });
    }
    ...
    private new() {
        this.isNew = true;
        this.timeCodeId = 0;
        this.timeCode = new TimeCodeAbsenceDTO();
        this.timeCode.type = SoeTimeCodeType.Absense;
    ...
    private reloadNavigationRecords(selectedRecord) {
        ...
        this.timeService.getTimeCodesGrid(SoeTimeCodeType.Absense, false, true).then(data => {
            _.forEach(data, (row) => {
