...
export class GridController extends GridControllerBase2Ag implements ICompositionGridController {
    ...
    constructor(
        ...
        super(gridHandlerFactory, "Time.Time.TimeCodeWorks", progressHandlerFactory, messagingHandlerFactory);
    ...
    private setupGrid() {
        const keys: string[] = [
            ...
            "time.time.timecode.classification"
        ];
        ...
        return this.translationService.translateMany(keys).then(terms => {
            ...
            this.gridAg.addColumnText("classificationText", terms["time.time.timecode.classification"], null);
            this.gridAg.addColumnText("description", terms["common.description"], null, true);
            this.gridAg.addColumnText("payrollProductNames", terms["time.payrollproduct.payrollproducts"], null, true);
            this.gridAg.addColumnEdit(terms["core.edit"], this.edit.bind(this), false);

            this.gridAg.finalizeInitGrid("time.time.timecodeworks.timecodeworks", true, undefined, true);
        });
    }
    ...
    public loadGridData(useCache: boolean) {
        this.progress.startLoadingProgress([() => {
            return this.timeService.getTimeCodesGrid(SoeTimeCodeType.Work, false, true).then(x => {
                return x;
            }).then(data => {
                this.setData(data);
            });
    ...
    private save(items: number[]) {
        ...
        if ((dict !== undefined) && (Object.keys(dict).length > 0)) {
            this.timeService.updateTimeCodesState(dict).then(result => {
                if (!result.success) {
                    const modal = this.notificationService.showDialogEx('', result.errorMessage, SOEMessageBoxImage.Error);
                    modal.result.then(() => {
                        this.reloadData();
                    });
                } else
