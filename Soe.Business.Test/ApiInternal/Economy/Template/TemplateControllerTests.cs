...
namespace Soe.Business.Test.ApiInternal.Economy.Template
{
    ...
    {
        public static int actorCompanyId = 1479674;
        public static int userId = 74370;
        public static int licenseId = 1;
        public static int roleId = 7635;
        ...
        public void TestGetTemplates()
        {
            ...
            var voucherTemplatesResult = controller.GetVoucherTemplatesCopyItems(actorCompanyId);
            var response = voucherTemplatesResult.ExecuteAsync(CancellationToken.None).Result;
            var voucherTemplates = (List<VoucherTemplatesCopyItem>)response.Content.ReadAsAsync(typeof(List<VoucherTemplatesCopyItem>)).Result;

            var voucherTemplate = voucherTemplates.Where(x=>x.Text == "Voucher template 1").FirstOrDefault();

            #region Assert

            Assert.IsTrue(voucherTemplates.Count > 0);
        ...
        [TestMethod]
        public void TestSupplierCopyItems()
        {
            var economyTemplateManager = new EconomyTemplateManager(base.GetParameterObject(actorCompanyId, userId, roleId));
            var upplierCopyItem = economyTemplateManager.GetSupplierCopyItemsFromApi(21, 1446907);
        }
    }
}
