...
{
    public class CoopCustomerInvoice
    {
        public string ApplyCoopCustomerInvoiceSpecialModification(string content, int actorCompanyId)
        {
            //Method to create standard invoice based on COOP:S ERK format
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                customerNumber = line.Substring(102, 4);
                
                string receiptNumber = line.Substring(69, 4);
                string date = line.Substring(2, 6);
                decimal amount = line.Substring(40, 13) != string.Empty ? Convert.ToDecimal(line.Substring(40, 13)) / 100 : 0;
                string sign = line.Substring(53, 1);
                decimal vat = line.Substring(41, 8) != string.Empty ? Convert.ToDecimal(line.Substring(41, 8)) / 100 : 0;
                ...
                if (line.Length != 49)
                {
                    ...
                    vatFreeAmount = line.Substring(73, 8) != string.Empty ? Convert.ToDecimal(line.Substring(65, 8)) / 100 : 0;
                }
            ...
            if (hasProductRows) // Customerinvoice with detail VAT and productrows - different format
            {
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    if (line.StartsWith("K") && ( previousWasProductRow || customerInvoiceRows.Any()))
                    {
                        string[] inputRow = line.Split(delimiter);

                        //We need to add the previousInvoices to the list
                        if (customerInvoiceRows.Any())
                        {
                        ...
                        //Parse information              
                        customerNumber = inputRow[2] != null ? inputRow[2] : string.Empty;
                        string date = inputRow[3] != null ? inputRow[3] : string.Empty;
                        ...
                        if ((vat6 + vat12 + vat25 + vatFreeAmount) != 0)
                        {
                            ...
                            {
                                amountDiff = amount - (priceVat6Rounded + priceVat12Rounded + priceVat25Rounded + vatFreeAmount) - vat;
                        ...
                        customerInvoice.Add(
                            ...
                            new XElement("FakturaAdressCo", customerBillingAddressCO),
                        ...
                        if (vat25 != 0)
                        {
                            ...
                            string price = Math.Round(Convert.ToDecimal((vat25 / backwardVat25) - vat25), 2).ToString();
                        ...
                        if (vat12 != 0)
                        {
                            ...
                            string price = Math.Round(Convert.ToDecimal((vat12 / backwardVat12) - vat12), 2).ToString();
                    ...
                    if (line.StartsWith("V"))
                    {
                        ...
                        string quantity = inputRow[4] != null ? inputRow[4] : string.Empty;
                        quantity = quantity.Replace(".000", "");

                        customerInvoiceRow.Add(
                            new XElement("Artikelid", string.Empty),
                            new XElement("Namn", "Vara: " + name + " Antal: " + quantity),
                            new XElement("Antal", 1),
                            new XElement("Pris", 0),
                            new XElement("Momskod", string.Empty),
                            new XElement("MomsProcent", 0),
            ...
            foreach (XElement customerInv in sortedCustomerInvoices)
            {
                numberOfInvoices += 1;
