...
export class StaffBasePage extends BasePage {
  ...
  async SetupStaff(envUrl: string, role: string) {
    await this.GoTo(envUrl);
    await this.SetRole(role);
    await this.changeLanguage();
    await this.GoToStaff(envUrl);
  }

  async GoToStaff(baseUrl: string) {
    await allure.step("Go to staff ", async () => {
      await this.page.goto(baseUrl + '/soe/time');
