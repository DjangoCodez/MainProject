...
test(testCaseId + ': Create Agreement To Preliminary Invoice : AP  ', { tag: ['@Sales', '@Agreements', '@Regression'] }, async ({ salesBasePage, agreementsPage, invoicePageJS }) => {
    ...
    await agreementsPage.createPreliminaryInvoicefromAgreement();
    await salesBasePage.goToMenu('Invoice', 'Invoices');
    await invoicePageJS.Reload();
    await invoicePageJS.filterByInternalText(internalText_1);
    await invoicePageJS.verifyInvoice();
    await salesBasePage.goToMenu('Agreements ', 'Agreements');
    ...
    await agreementsPage.createPreliminaryInvoice();
    await salesBasePage.goToMenu('Invoice', 'Invoices');
    await invoicePageJS.filterByInternalText(internalText_2);
    await invoicePageJS.verifyInvoice();
});
