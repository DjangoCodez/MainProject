...
test(testCaseId + ': Agreements finish : SR', { tag: ['@Sales', '@Agreements', '@Regression'] }, async ({ salesBasePage, agreementsPage }) => {
    ...
    await agreementsPage.clickFinishButton();
    await agreementsPage.close();
    await agreementsPage.moveToAgreementTab();
    await agreementsPage.filterByInternalTextAgreements(internalText);
    await agreementsPage.VerifyRowCount(0);
});
