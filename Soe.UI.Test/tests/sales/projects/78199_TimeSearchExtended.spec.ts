...
test(testCaseId + ': Verify time search extended : SR', { tag: ['@Sales', '@Projects', '@TimeReport', '@Regression'] }, async ({ timeReportPage, orderPageJS }) => {
    ...
    await timeReportPage.clickAddRow();
    await timeReportPage.setRegisterTimeDetails({ employeeName: employee, orderNumber: capturedOrderNumber, projectNumber: capturedProjectNumber, chargingType: 'Övertid efter kl 24 samt helg', timeWorked: '5', billableTime: '2', rowIndex: 0 });
    await timeReportPage.saveRegisteredTime();
