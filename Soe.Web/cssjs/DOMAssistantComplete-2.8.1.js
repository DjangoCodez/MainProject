...
var DOMAssistant = function () {
	...
	return {
			...
			for (var i=0, arg, idMatch; (arg=arguments[i]); i++) {
				if (typeof arg === "string") {
					...
					if (regex.id.test(arg)) {
						...
						elm = (!document.querySelectorAll && useCache && lastCache.rule && lastCache.rule === arg && lastCache.doc === doc) ? lastCache.elms : pushAll(elm, DOMAssistant.cssSelectionDOM.call(document, arg));
		...
		cssSelectionDOM : function (cssRule) {
		...
		cssSelect : function (cssRule) {
		    return DOMAssistant.cssSelectionDOM.call(this, cssRule);
		},
		...
		elmsByTag : function (tag) {
		    return DOMAssistant.cssSelectionDOM.call(this, tag);
		}
	};
}();
