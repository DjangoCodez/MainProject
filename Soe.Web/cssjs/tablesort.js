...
fdTableSort = {
        ...
        init: function(tableId) {
                ...
                var c, ii, len, onLoadSort, colMatch, showOnly, match, showArrow, columnNumSortObj, thNode, obj, workArr, headers, thtext, aclone, multi, colCnt, cel, allRowArr, rowArr, sortableTable, celCount, colspan, rowspan, rowLength;
                        ...
                        for(c = 0;c < workArr.length;c++) {
                                for(i = 0;i < workArr[c].length;i++){
                                        ...
                                        if(workArr[c][i].className.match('sortable')) {
                                                ...
                                                //TODO: Hard-coded
                                                aclone.title = "Sortera efter \u201c" + thtext + "\u201d";
        ...
        onUnload: function() {
                for(var tbl1 in fdTableSort.tableCache) {
                        fdTableSort.removeTableCache(tbl1);
                };
                for(var tbl2 in fdTableSort.tmpCache) {
                        fdTableSort.removeTmpCache(tbl2);
                };
        ...
        addThNode: function() {
                ...
                if(dataObj.thList.length > 1) {
                        var classToAdd = fdTableSort.thNode.className.search(/forwardSort/) != -1 ? "reverseSort" : "forwardSort";
        ...
        dateFormat: function(dateIn, favourDMY) {
                ...
                while(cnt < numFormats) {
                        ...
                        if(dateIn.match(dateTest[start].regExp)) {
                                var res = dateIn.match(dateTest[start].regExp);
                                var y = res[dateTest[start].y];
                                var m = res[dateTest[start].m];
                                var d = res[dateTest[start].d];
